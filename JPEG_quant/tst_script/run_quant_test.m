function [stats, enc_opts, dec_opts, raw_img, raw_nq_img] =...
    run_quant_test(img_inp_dir, img_out_dir, unquant, prll)
  if nargin < 3
    unquant = false;
  end
  if nargin < 4
...
%     );
  ...
  enc_opts1a = struct(...
    'msrmnt_input_ratio', {num2cell(csr)}, ...
    'msrmnt_mtrx', struct('type', {'WH', 'DCT' }),...
    'qntzr_wdth_rng_exp', 2,...
    'qntzr_wdth_mltplr', num2cell(q_step), ...
    'qntzr_wdth_mode', CS_EncParams.Q_WDTH_CSR ...
    );
  enc_opts1b = struct(...
    ...
    'msrmnt_mtrx', struct('type', {'WH', 'DCT' }),...
    'qntzr_wdth_rng_exp', 2,...
    'qntzr_wdth_mltplr', num2cell(q_step*0.05), ...
    'qntzr_wdth_mode', CS_EncParams.Q_WDTH_RNG_CSR ...
    );
  enc_opts2 = struct(...
    'msrmnt_input_ratio', {num2cell(csr)}, ...
  ...
  enc_opts = {enc_opts1b, enc_opts1a, enc_opts2};
