function [stats, enc_opts, dec_opts, raw_img, raw_nq_img] = ...
    run_quant_test_cscd_test_2DSRM_WHTDCT_NLR_CS(img_inp_dir, img_out_dir, img_files,unquant, prll)
  if nargin < 4
    unquant = false;
  end
  if nargin < 5
...
enc_opts = struct(...
    ...
    'msrmnt_mtrx', struct('type', {'MD_WH' 'MD_DCT', 'WH', 'DCT' },'nxt',{[]}),...
