classdef CS_DecParams < ProcessingParams
    ...
    properties
      ...
      % Decode together all the blocks that constitute a frame.      % IMPLEMENTED YET.
      whole_frames = 0;
      
      % Exansion level at which the solution is computed
      % (VidBlocker.BLK_STT_xxx)
      expand_level = VidBlocker.BLK_STT_INTRPLT;
      ...
      % If the lagrangian difference between after one internal iteration is
      % less than than the previous difference by a factor of less than 
      % step_ratio_thrsh, stop inner loop.
      step_ratio_thrsh = 0.9;
      ...
      eps_lgrng_chg_rate = 0.7; % Scaler of threshold when reached.
      eps_lgrng_chg = 0.01;   % Lowest threshold
      ...
      sparsifier = struct('type', 'VidSparserTV_DCT', ...
        'args', struct(...
        'mode',0, 'b_stt', ...
        VidBlocker.BLK_STT_INTRPLT,...
        'motn_spec', struct(),... 
        'motn_crct', 0 ... % start correcting for motion after %d checks. 0=never.
        ));
      
      % beta parameters:
      % - rate control for penalty coefficients. Recommended > 2
      beta_rate = 1.2;
      beta_rate_thresh = 0.2;  % DEPRECATED
