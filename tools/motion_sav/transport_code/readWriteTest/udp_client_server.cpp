    ...
    , f_addr(addr)
{
  ...
  tv.tv_sec = 5;
  tv.tv_usec = 0;
  if (setsockopt(f_socket, SOL_SOCKET, SO_RCVTIMEO,&tv,sizeof(tv)) < 0) {
      perror("setting socket opt Error");
  }
#endif
    if(f_socket == -1)
    {
...
int udp_server::timed_recv(char *msg, size_t max_size, int max_wait_ms)
{
    ...
    int retval = select(f_socket + 1, &s, NULL, &es, &timeout);
    std::cerr<<"## retval= "<<retval<<" " <<FD_ISSET(f_socket, &s)<<" "<<FD_ISSET(f_socket, &es)<<std::endl;
    if(retval > 0)
    {
        //alarm(6);    
        // our socket has data
        int nre = ::recv(f_socket, msg, max_size, 0);
        cerr<<"nre ="<<nre<<endl;
        return nre;
    }
