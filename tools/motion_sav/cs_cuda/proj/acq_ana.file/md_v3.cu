...
#define DBG_MT_STEP4_1		0x100000
...
// stat
static unsigned int total_d_mem = 0 ;
...
static struct cube wcube_info_v3[ CUBE_INFO_CNT ] ;	// a working copy from cube_info above
...
static struct cube *d_cubep = NULL ;
...
enum {
	...
	CS_TIMER_ANALYSIS_MD4_1,	// the following 4 have to consecutive
	CS_TIMER_ANALYSIS_MD4_2,
	CS_TIMER_ANALYSIS_MD4_3,
	CS_TIMER_ANALYSIS_MD4_4,
	CS_TIMER_COUNT
} ;
...
main( int ac, char *av[] )
{
	...
	// STEP 0 ::: read the config file in .json
	...
	fprintf( stderr, "x/y (%d, %d) blk x/y/z ( %d, %d, %d) out %s yonly %d "
		"swap %d\n",
	...
	// STEP 2 ::: init the dbg routines

	dbg_init ( 256 * 1024 * 1024 * sizeof ( int )) ;

	// STEP 3 ::: initialize the memory on gpu
	...
	// STEP 4 ::: initialize the memory on host
	...
	// STEP 5 ::: initialize the video input

	if ( csc.capture )
	{
		printf("warning: capture is reset to 0\n") ;
		csc.capture = 0 ;
	}
	...
	if ( !cs_vio_init ( csc.z_block, csc.frame_x, csc.frame_y, csc.video_src,
	   ...
	   csc.fps, csc.do_display, csc.ignore_edge, csc.capture, csc.display_threshold ))
	{
	...
	fprintf( stderr, "%s: size of header %lu\n", __func__,
		 (unsigned long)sizeof ( struct cs_header )) ;
	...
	// STEP 6 ::: initialize the cvs output file header
	...
	// STEP 7 ::: initialize the motion detect output file header

	if ( md_filep )
		ma_report_header ( md_filep, csc.frame_y, csc.frame_x, 0, 1, 2, 1 ) ;

	// STEP 8 ::: let the symphony start ...

	// LDL
	if ( !do_measurement( fout ))
	{
...
do_tst_longlong()
{
  fprintf(stderr, "%s: size of ll %lu\n", __func__, (unsigned long)sizeof ( long long )) ;
...
allocate_d_mem()
{
	int ana_size=0, ocube_size = 0, cube_size, *hdp = NULL, nmea, nz,
		i, j, k, xx, yy, zz, x, y, z ;
		
	if ( csc.do_cube )
	{
		if (( k = cudaMalloc( &d_cubep, sizeof ( *d_cubep ) *
			CUBE_INFO_CNT + sizeof ( int ) * 10 )) != cudaSuccess )
		{
			printf("%s: cubep alloc failed %d\n", __func__, k ) ;
			return ( 0 ) ;
		}

		total_d_mem += sizeof ( *d_cubep ) * CUBE_INFO_CNT + sizeof ( int ) * 10 ;

		d_host_io = ( int * )( d_cubep + CUBE_INFO_CNT ) ;

		fprintf( stderr, "%s: d_cubep %p d_host_io %p\n",
			__func__, d_cubep, d_host_io ) ;
		...
		for ( i = 0 ; i <  CUBE_INFO_CNT  ; i++ )
		{
			...
			if (( x * y * z * (int)sizeof ( int )) > ocube_size )
			{
			...
			// we can only do one block at a time for md ... 
			// each column will have (x * y * ( cube_info[i].z - 1 ) + NUM_OF_HVT_INDEX )
			// will have this many rows 
		   	// ((( csc.md_x << 1 ) + 1 ) * (( csc.md_y << 1 ) + 1) * csc.md_z + 1 )
			// the last one is for no motion 	

			if ( cube_info[i].z > csc.md_z )
			{
				cube_info[i].md_v3_cnt = csc.md_z ;	// chunks of hv blocks for the md operaton
				cube_info[i].md_v3_hv_cnt = cube_info[i].z - csc.md_z ; // each record will have
					// this many of hv frames
			} else
			{
				cube_info[i].md_v3_cnt = cube_info[i].z - 1 ;// chunks of hv blocks for the md operaton
				cube_info[i].md_v3_hv_cnt = 1 ;// each record will have
			}

			k = ( x * y * ( cube_info[i].md_v3_hv_cnt ) + NUM_OF_HVT_INDEX ) *
				((( csc.md_x << 1 ) + 1 ) * (( csc.md_y << 1 ) + 1) * cube_info[i].md_v3_cnt + 1 ) ;
				// last + 1 is for orig

			printf(": x %d y %d k %d ana_size %d\n", x, y, k, ana_size ) ;
			...
			if ( cube_info[i].z < 2 )
			{
				printf("%s :: error : i %d z size %d too small \n", __func__, i, cube_info[i].z ) ;
			   	return ( 0 ) ;
			}	
		...
		// set the config
		h_set_cube_config( d_cubep, cube_info ) ;

#ifdef CUDA_DBG 
		h_p_d_cube_config ( "config", d_cubep ) ;

		for ( k = 0 ; k < CUBE_INFO_CNT ; k++ )
			printf("k %d : x/y/z %d %d %d \n", k, cube_info[k].x,
				cube_info[k].y, cube_info[k].z ) ;
#endif 
	...
	if ( csc.do_reconstruction )
	{
		if ( !h_do_dct_init())
		{
			printf("allocated_d_mem: h_do_dct_init failed \n") ;
			return ( 0 ) ;
		}
	}
	...
	if ( csc.do_permutation )
		blk_size_e++ ;	// mlseq has to be 2^n -1 
	...
	blk_size_e = perm_blk_size_e = perm_size_e * nblk_in_x * nblk_in_y ;

	if ( perm_blk_size_e < ana_size )
	{
		printf("%s: error : size mismatch perm_blk_size_e %d ana %d\n",
			__func__, perm_blk_size_e, ana_size ) ;
		return ( 0 ) ;
	}	
	...
	{
		printf("%s: din_b2 failed %d -- size %d\n", __func__, i, blk_size_i ) ;
		return ( 0 ) ;
	}
	...
	printf("%s: DEV GOOD  == b1 %p b2 %p bsize %d \n"
		"	pR %p psize %d mllp %p mlrp %p total_d_mem %u\n",
	...
	for ( i = 0 ; i < CUBE_INFO_CNT ; i++ )
	{
		...
		fprintf( stderr, "%s: md_v3_cnt %d md_v3_hv_cnt %d\n",
			__func__,
			cube_info[i].md_v3_cnt,
			cube_info[i].md_v3_hv_cnt
			) ;
	}
...
allocate_h_mem()
{
	...
	if (( !ybufp && ( !strlen(csc.video_src))) || ( !csc.y_only && ( !ubufp || !vbufp )))
	{
...
setup_perm_tbls( int *d_bp )
{
...
#ifdef CUDA_OBS 
	dbg_perm_ck ( 123, ran, 1, d_bp, perm_size_e ) ;
#endif 
...
#ifdef CUDA_OBS 
	dbg_perm_ck ( 1234, ran, 1, dperm_rp, perm_size_e ) ;
#endif 
	...
	// for the mlseq LEFT perm ... 2^n - 1
	randi_RndC( &rnd_state_2, RndC_uint32( perm_size_e - 1 ), (unsigned int)1, &ran1 ) ; 
	...
	for ( i = 0 ; i < CUBE_INFO_CNT ; i++ )
	{
		h_do_perm_selection_L ( d_bp, perm_size_e - 1, cube_info[i].dp, // mlseq is 2^n -1 on LEFT
			cube_info[i].size, ran, cube_info[i].sink ) ;

#ifdef CUDA_OBS 
		dbg_perm_ck ( 789, ran, 0, d_bp, perm_size_e - 1 ) ;
#endif 

#ifdef CUDA_OBS 
		if ( i == 0 )
			dbg_p_d_data_i ("left selection : 0 ", d_bp, cube_info[0].size ) ;
		else if ( i== 1 )
			dbg_p_d_data_i ("left selection : 1 ", d_bp, cube_info[1].size ) ;
		else 
			dbg_p_d_data_i ("left selection : 2 ", d_bp, cube_info[2].size ) ;
#endif 
		h_do_permutation_double ( d_bp, dperm_ml_lp,
			cube_info[i].cube_perm, perm_size_e - 1 ) ;	// orig one

#ifdef CUDA_OBS 
		if ( i == 0 )
			dbg_p_d_data_i ("left double selection : 0 ", cube_info[i].cube_perm, perm_size_e ) ;
		else if ( i== 1 )
			dbg_p_d_data_i ("left double selection : 1 ", cube_info[i].cube_perm, perm_size_e ) ;
		else 
			dbg_p_d_data_i ("left double selection : 2 ", cube_info[i].cube_perm, perm_size_e ) ;
#endif 
...
int
do_measurement ( int fout )
{
	int cnt, i, frame_cnt ;
	double maxt=0, d, ad, aut, ast ;
	clock_t tut, tst ;
	...
	// this is the workhorse of the whole video acqusition and analysis

	omp_timer_init( CS_TIMER_COUNT ) ;
	cs_timer_init( 1 ) ;

	// STEP 0 ::: if so configured, start the camera for frame grabbings ...
	...
	while ( 1 )
	{
		// STEP 1 ::: setup the L and R permutation
		...
		{
#ifdef CUDA_OBS 
			printf("do_measurement: OUT === %d\n", frame_processed ) ;
#endif 
			fp = cs_vio_get() ;
#ifdef CUDA_OBS 
			printf("do_measurement: IN === %d\n", frame_processed ) ;
#endif 
			ybufp = fp->gbp ;

#ifdef CUDA_OBS 
			dbg_pr_h_first_last("read ybufp", ybufp, csc.frame_x * csc.frame_y, 10 ) ;
#endif 
		...
		if ( i == 2 )
		{
			...
			for ( i = 0 ; i < CS_TIMER_COUNT ; i++ )
			{
				...
				switch ( i ) {
				case CS_TIMER_ANALYSIS_MD0:
				case CS_TIMER_ANALYSIS_MD1:
				case CS_TIMER_ANALYSIS_MD2:
				case CS_TIMER_ANALYSIS_MD3:
				case CS_TIMER_ANALYSIS_MD4:
				case CS_TIMER_ANALYSIS_MD4_1:
				case CS_TIMER_ANALYSIS_MD4_2:
				case CS_TIMER_ANALYSIS_MD4_3:
				case CS_TIMER_ANALYSIS_MD4_4:

#ifdef CUDA_DBG 
					printf("%s ::: BEFORE %f cnt %d average %f ms frame %f\n", 
						timer_name[i], d, cnt, ad, ad / csc.z_block ) ;
#endif 
					cnt /= ( nblk_in_x * nblk_in_y ) ;
					ad *= ( nblk_in_x * nblk_in_y ) ;
				}

				printf("%s ::: %f cnt %d average %f ms frame %f ", 
					timer_name[i], d, cnt, ad, ad / csc.z_block ) ;
				...
				if ( ad > 0 && maxt > 0)
				{
					d = ( 1000.0 * ( double ) csc.z_block ) / ad ;
			...
			printf("overall st %d ut %d cnt %d ast %f aut %f == total %f ms\n",
			       int(tst), int(tut), cnt, ast, aut, ast + aut ) ; 
			return ( 1 ) ;
		}

#ifdef CUDA_OBS 
		frame_processed += csc.z_block ;

		printf("DO frame_process block %d \n", frame_processed ) ;
#endif 

		// STEP 3 ::: set the cube info to the gpu mem

		memcpy( wcube_info, cube_info, sizeof ( cube_info )) ;
		h_set_cube_config ( d_cubep, wcube_info ) ;

		// do y
	
		// STEP 4 ::: do one component ... y 
...
fp:	frame_list pointer ... for ipcam/file/webcam 
*/
	...
	int need_interpolate, struct frame_list *fp )
{
	...
	int md_v3_cnt, hvt_idx, k, frame_size = x * y ;
	float *flp ;
	int *ip ;
	...
	// STEP 1 ::: copy the frame to the device
	...
	// STEP 2 ::: change the one byte pixel value to int

	h_cast_T_to_T<unsigned char, float>(( unsigned char * ) d_currp, ( float *)d_nextp, in_size ) ; 
	...
	if ( csc.dbg_flag & DBG_C_2_I )
		dbg_p_d_data_f_mn ( "after c to f", ( float *)d_currp, in_size, 
			x, y, x ) ; 
	...
	h_expand_frame_T<float, float> ((float *)d_currp, ( float *)d_nextp,
	...
	if ( csc.dbg_flag & DBG_EXPAND )
	{
		...
		dbg_p_d_data_f_mn ( "after expand", ( float *)d_currp, i, 
			x + ( csc.xadd << 1 ),
			y + ( csc.yadd << 1 ), 
			x + ( csc.xadd << 1 )) ;
	}
	...
	if ( csc.do_block )
	{
		...
		h_make_block<float>( ( float *)d_currp, ( float *)d_nextp, 
...
#ifdef CUDA_OBS 
			dbg_p_d_data_f_mn_skip ( "after blking",
...
#endif 
#ifdef CUDA_DBG 
			dbg_p_d_data_f_mn ( "after blking",
				csc.do_permutation ? ( float *)d_currp + 1 : ( float *)d_currp, 
				perm_blk_size_e, 
				xbdim + csc.adj_x, ybdim + csc.adj_y, xbdim + csc.adj_x ) ;
#endif 
	...
	if ( csc.do_permutation )
	{
		...
		// input is d_currp ... 
		h_do_permutation_R<float> (( float *)d_currp, ( float *)d_nextp, dperm_rp, perm_blk_size_e,
			perm_size_e ) ;
		...
		if ( csc.dbg_flag & DBG_PERM_R )
			dbg_p_d_data_f_mn_skip ( "after perm-R",
				csc.do_permutation ? ( float *)d_currp + 1 : ( float *)d_currp, 
				perm_blk_size_e, 
				xbdim + csc.adj_x, ybdim + csc.adj_y, csc.z_block + csc.zadd,
				xbdim + csc.adj_x, perm_size_e ) ;
	...
	if ( csc.dbg_flag & DBG_WHM )
		dbg_p_d_data_f_mn_skip ( "before whm",
			csc.do_permutation ? ( float *)d_currp + 1 : ( float *)d_currp, 
			perm_blk_size_e, 
			xbdim + csc.adj_x, ybdim + csc.adj_y, csc.z_block + csc.zadd,
			xbdim + csc.adj_x, perm_size_e ) ;
	...
	cs_whm_measurement_b<float>( ( float *)d_currp, perm_blk_size_e, perm_size_e ) ; 

	omp_timer_off( CS_TIMER_MEA ) ;

	if ( csc.dbg_flag & DBG_WHM )
		dbg_p_d_data_f_mn_skip ( "after whm",
			csc.do_permutation ? ( float * )d_currp + 1 : ( float * )d_currp, 
			perm_blk_size_e, 
			xbdim + csc.adj_x, ybdim + csc.adj_y, csc.z_block + csc.zadd,
			xbdim + csc.adj_x, perm_size_e ) ;
	...
	if ( csc.do_permutation )
	{
		...
		h_do_permutation_Lv2<float> ( ( float *)d_currp, ( float *)d_nextp,
		...
		if ( csc.dbg_flag & DBG_PERM_L )
			dbg_p_d_data_f_mn_skip ( "after L-perm",
				csc.do_permutation ? (float *)d_currp + 1 : (float *)d_currp, 
				perm_blk_size_e, 
				xbdim + csc.adj_x, ybdim + csc.adj_y, csc.z_block + csc.zadd,
				xbdim + csc.adj_x, perm_size_e ) ;
	...
	// so for the corner/side cubes, the inner_cube_size is bigger than it actually is.

	if ( !( k = h_do_copy_vec<float> ( ( float *)d_currp, ( float *)d_nextp,
		overall_size, 
		perm_size_e,
		inner_cube_size )))
	{
	...
	{
		dbg_p_d_data_f_mn_v2 ( "copy after", ( float *)d_currp, overall_size, wcube_info[0].x,
	...
	if ( csc.do_analysis )
	{
		...
		h_do_edge_detection_v2<float> (( float *)d_currp, ( float *)d_nextp, overall_size,
			d_cubep, csc.edge_x, csc.edge_y, nblk_in_x, nblk_in_y,
			cube_info) ;
		...
		{
			dbg_p_d_data_f_mn_v2 ( "edge_v2 orig", ( float *)d_currp, overall_size, wcube_info[0].x,
				wcube_info, nblk_in_x, nblk_in_y ) ;
			dbg_p_d_data_f_mn_v2 ( "edge_v2 get", ( float *)d_nextp, overall_size, wcube_info[0].x,
				wcube_info, nblk_in_x, nblk_in_y ) ;
		}
		...
		if (!(i = h_do_copy_box_v2<float> (( float *)d_currp, ( float *)d_nextp, overall_size,
			csc.edge_x, csc.edge_y, nblk_in_x, nblk_in_y, d_cubep, cube_info )))
		{
...
#ifdef CUDA_OBS 
			printf("wcube info %d x %d y %d z %d size %d \n",
				i, 
				wcube_info[i].x,
				wcube_info[i].y,
				wcube_info[i].z,
		...
		if ( csc.dbg_flag & DBG_ED )
		{
			dbg_p_d_data_f_mn_v2 ( "edge_v2 after copy", (float *)d_currp, overall_size, wcube_info[0].x,
				wcube_info, nblk_in_x, nblk_in_y ) ;
		}

		// motion detection 
		/* from this point on d_currp will have the edge-detected data, data will
		   copy to the d_nextp, one block at a time, then process till it is done
		  */

		for ( i = 0 ; i < nblk_in_x * nblk_in_y ; i++ )
		{
			// V3 : loop thru all blocks

			memcpy ( &wcube_info_v3, &wcube_info, sizeof ( wcube_info )) ;

			// currp has the edge block data ...
			// we will build the idx table (TVH, etc) for the motion dection 
			// in the nextp

			k = i / nblk_in_x ;

			if (( k == 0 ) || ( k == ( nblk_in_y -1 )))
			{
				k = i % nblk_in_x ;
				if (( k == 0 ) || ( k == ( nblk_in_x - 1 )))
					hvt_idx = CUBE_INFO_CORNER ;
				else
					hvt_idx = CUBE_INFO_SIDE ;
			} else
			{
				k = i % nblk_in_x ;
				if (( k == 0 ) || ( k == ( nblk_in_x - 1 )))
					...
					hvt_idx = CUBE_INFO_INNER ;
			}

			printf("big loop: i %d hvt_idx %d blk_in_x/y %d %d md_v3_cnt %d\n",
				i, hvt_idx, nblk_in_x, nblk_in_y, wcube_info_v3[hvt_idx].md_v3_cnt ) ;

#ifdef CUDA_DBG 
			set_device_mem(( float *)d_nextp, perm_blk_size_e, 1.01 ) ;
#endif 

			k = h_do_motion_idx_v3 ( ( int *)d_nextp, perm_blk_size_e, &orig,
				wcube_info_v3, csc.md_x, csc.md_y, csc.md_z,
				&rec_size, hvt_idx ) ;
			...
			// hvt_size is the total records count in this blk.
			md_v3_cnt = wcube_info_v3[ hvt_idx ].md_v3_cnt ;

			hvt_size = ( csc.md_x * 2 + 1 ) * ( csc.md_y * 2 + 1 ) * md_v3_cnt + 1 ; // 1 is for orig
			
			if ( csc.dbg_flag & DBG_MT_IDX )
				printf("orig idx is %d size %d hvt_size %d\n", orig, rec_size, hvt_size ) ;

			// the following data has a size issue ... it is too big ... 
			// for pets_test_enc case QQQ

			if ( csc.dbg_flag & DBG_MT_IDX )
				cs_p_d_tvh ("idx original", d_nextp, rec_size, hvt_size, 8, rec_size -7, 0 ) ;
#ifdef CUDA_OBS 
				dbg_p_d_data_f_mn ( "idx original ", ( float *)d_nextp,
					( rec_size + NUM_OF_HVT_INDEX ) * hvt_size,
					( rec_size + NUM_OF_HVT_INDEX ), 
					hvt_size, 6 ) ;
#endif 

			if ( csc.dbg_flag & DBG_MT_STEP0 )
			{
				k = wcube_info_v3[ hvt_idx ].x * wcube_info_v3[ hvt_idx ].y *
					wcube_info_v3[ hvt_idx ].z ;

				printf("after EDGE size %d, x %d y %d z %d \n", 
					k, wcube_info_v3[ hvt_idx ].x,wcube_info_v3[ hvt_idx ].y,
					wcube_info_v3[ hvt_idx ].z ) ;

				dbg_p_d_data_f_mn ( "after EDGE", ( float *)d_currp + i * inner_cube_size,
					k, 
					wcube_info_v3[ hvt_idx ].x,
					wcube_info_v3[ hvt_idx ].y * wcube_info_v3[ hvt_idx ].z,
					wcube_info_v3[ hvt_idx ].x );
			}
			...
			// step 0 : copy data ...
			// currp has the valid edge data ... nextp has the valid (TVH) formated data
			k = h_do_motion_detection_step0_v3<float> (( float *) d_currp + i * inner_cube_size,
				( float *) d_nextp,
				rec_size * hvt_size,
				wcube_info_v3[ hvt_idx ].x, 
				wcube_info_v3[ hvt_idx ].x * wcube_info_v3[ hvt_idx ].y, 
				wcube_info_v3[ hvt_idx ].x - ( csc.md_x << 1 ), 
				( wcube_info_v3[ hvt_idx ].x - ( csc.md_x << 1 )) *
					( wcube_info_v3[ hvt_idx ].y - ( csc.md_y << 1 )),
				wcube_info_v3[ hvt_idx ].md_v3_hv_cnt ) ;

			omp_timer_off ( CS_TIMER_ANALYSIS_MD0 ) ;

			if ( csc.dbg_flag & DBG_MT_IDX )
				cs_p_d_tvh ("idx after step0", d_nextp, rec_size, hvt_size, rec_size, 0, 0 ) ;
#ifdef CUDA_OBS 
				dbg_p_d_data_f_mn ( "idx after step0", ( float *)d_nextp,
					( rec_size + NUM_OF_HVT_INDEX ) * hvt_size,
					( rec_size + NUM_OF_HVT_INDEX ), 
					hvt_size, 6 ) ;
#endif 
			...
			// the per motion block is in d_nextp,
			// the edged block is in d_currp ;
			// never change again, till this block-frames are done
			
			if ( csc.dbg_flag & DBG_MT_STEP0 )
				dbg_p_d_data_f_mn ( "motion 0", ( float *)d_nextp,
					( rec_size + NUM_OF_HVT_INDEX ) * hvt_size,
					rec_size + NUM_OF_HVT_INDEX, hvt_size, rec_size + NUM_OF_HVT_INDEX ) ;

			// step 1 ... yk-y0 LDL ...  it will repeat for each block ... BAD

			for ( k = 0 ; k < CUBE_INFO_CNT ; k++ )
			{
				wcube_info_v3[k].x -= csc.md_x * 2 ;
				wcube_info_v3[k].y -= csc.md_y * 2 ;
				wcube_info_v3[k].z -= ( csc.md_z - 1 ) ;

				wcube_info_v3[k].size = wcube_info_v3[k].x * wcube_info_v3[k].y * wcube_info_v3[k].z ;
			}

			h_set_cube_config ( d_cubep, wcube_info_v3 ) ;

			if ( csc.dbg_flag & DBG_MT_STEP1 )
			{
				cs_p_d_tvh ("step 1 before", d_nextp, rec_size, 3, 8, rec_size -7, 0 ) ;
				cs_p_d_tvh ("step 1 before tail", d_nextp, rec_size, 3, 8, rec_size -7, orig - 3 ) ;
				cs_p_d_tvh ("step 1 before", d_nextp + orig *( rec_size + NUM_OF_HVT_INDEX),
					rec_size, 1, 8, rec_size-7, 0 ) ;
			}
			
			omp_timer_on ( CS_TIMER_ANALYSIS_MD1 ) ;

			h_do_l1_norm_step1_v3<float>((float *) d_nextp, 
				rec_size * ( hvt_size - 1 ),	// do not do y0-yk for orig 
				rec_size, orig ) ;
			...
			if ( csc.dbg_flag & DBG_MT_STEP1 )
			{
				cs_p_d_tvh ("step 1 after", d_nextp, rec_size, 3, 8, rec_size -7, 0 ) ;
				cs_p_d_tvh ("step 1 after tail", d_nextp, rec_size, 3, 8, rec_size -7, orig - 3 ) ;
				cs_p_d_tvh ("step 1 after", d_nextp + orig *( rec_size + NUM_OF_HVT_INDEX),
					rec_size, 1, 8, rec_size-7, 0 ) ;
			}

			if ( csc.dbg_flag & DBG_MT_STEP2 )
			{
				cs_p_d_tvh ("step 2 before", d_nextp, rec_size, 2, rec_size, 0 ) ;
				cs_p_d_tvh ("step 2 before ORIG", d_nextp, rec_size, 1, rec_size, 0, orig ) ;
			}
			// step 2 -- do the sum

			omp_timer_on ( CS_TIMER_ANALYSIS_MD2 ) ;

			k = h_do_l1_norm_step2_v3<float>( ( float *)d_nextp, rec_size, hvt_size ) ;
			...
			if ( csc.dbg_flag & DBG_MT_STEP2 )
			{
				cs_p_d_tvh ("step 2 after", d_nextp, rec_size, 2, rec_size, 0 ) ;
				cs_p_d_tvh ("step 2 after ORIG", d_nextp, rec_size, 1, rec_size, 0, orig ) ;
			}

			if ( csc.dbg_flag & DBG_MT_STEP3 )
			{
				cs_p_d_tvh ("step 3 before", d_nextp, rec_size, 20, 1, 0, 0 ) ;
				cs_p_d_tvh ("step 3 before", d_nextp, rec_size, 20, 1, 0, hvt_size -1 -20 ) ;
				cs_p_d_tvh ("step 3 before ORIG", d_nextp, rec_size, 1, rec_size, 0, orig ) ;
			}
			...
			k = h_do_l1_norm_step3_v3<float>((float *)d_nextp, rec_size, orig, hvt_size - 1 ) ;
				// the 1 is for orig ...
			...
			if ( csc.dbg_flag & DBG_MT_STEP3 )
			{
				cs_p_d_tvh ("step 3 after", d_nextp, rec_size, 20, 1, 0, 0 ) ;
				cs_p_d_tvh ("step 3 after", d_nextp, rec_size, 20, 1, 0, hvt_size -1 -20 ) ;
				cs_p_d_tvh ("step 3 after ORIG", d_nextp, rec_size, 1, 1, 0, orig ) ;
			}
			...
			{
				cs_p_d_tvh ("step 4 before", d_nextp, rec_size, hvt_size, 1, 0, 0 ) ;
			}

			omp_timer_on ( CS_TIMER_ANALYSIS_MD4 ) ;

			k = h_do_l1_norm_step4_v3<float>( ( float *)d_nextp,
				rec_size, orig, hvt_size - 1, outbufp + i * ( L1_NORM_STEP4_RETURN_ENTRY_SIZE *
					2 ), ((( csc.md_x * 2 + 1 ) * ( csc.md_y * 2 + 1 )) / 2), CS_TIMER_ANALYSIS_MD4_1) ;
				...
				// csc.md_y * 2 is for top and bottom, +1 for the special record at the
				// end ( i.e. orig ), +1 for the V=md_y H=md_x T=x
			...
			// should be still int in the following 3 dbg_p_d_data_f_mn statements

			if ( csc.dbg_flag & DBG_MT_STEP4 )
			{
				cs_p_d_tvh ("step 4 afater", d_nextp, rec_size, hvt_size, 1, 0, 0 ) ;
			}

			if ( csc.dbg_flag & DBG_MT_STEP4_1 )
			{
				flp = ( float *)outbufp ;
				ip = outbufp ;

				flp += NUM_OF_HVT_INDEX ;

				printf("STEP 4 so far ... %d outbufp %p\n", i, outbufp ) ;
				
				for ( k = 0 ; k <= i ; k++ )
				{
					printf(" --- %d --- t %d v %d h %d v %f == t %d v %d h %d v %f \n",
						k,
						ip[0],
						ip[1],
						ip[2],
			...
			// LDL update the outbufp accordingly ...
		}
...
cs_config_check( struct cs_config *csp )
{
	...
	if ( csp->do_analysis && (
		...
		( csp->md_z <= 0 )))
	{
		fprintf( stderr, "negative motion detection size %d %d %d do_analysis %d \n",
			csp->md_x, csp->md_y, csp->md_z, csp->do_analysis ) ;
		err++ ;
	}
	...
	if ( csp->video_src && !csp->y_only )
	{   
		fprintf( stderr, "ipcam/webcam/file but not y_only\n") ;
		err++ ;
	}   
	...
	if (( csp->frame_x < 0 ) || ( csp->frame_y < 0 ) ||
		...
		!strlen(csp->video_src) || !strlen(csp->foutname) ||
		!csp->video_source)
	{
