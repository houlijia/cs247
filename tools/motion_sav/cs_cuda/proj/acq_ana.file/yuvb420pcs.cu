	...
	int need_interpolate, struct frame_list *fp )
{
	...
	if ( csc.do_analysis )
	{
		...
		k = h_do_motion_idx_v2 ( ( int *)d_nextp, blk_size_e, &orig,
		...
		// hvt_size is loopcnt in h_do_motion_idx_v2
		...
		if ( csc.dbg_flag & DBG_MT_IDX )
			dbg_p_d_data_f_mn ( "idx original ", ( float *)d_currp,
		...
		k = h_do_motion_detection_step0_v2<float> (( float *) d_currp,( float *) d_nextp,
		...
		if ( csc.dbg_flag & DBG_MT_STEP0 )
			dbg_p_d_data_f_mn ( "motion 0", ( float *)d_currp,
				( rec_size + NUM_OF_HVT_INDEX ) * hvt_size * nblk_in_x * nblk_in_y, 
		...
		h_do_l1_norm_step1_v2<float>((float *) d_currp, 
			rec_size * hvt_size * nblk_in_x * nblk_in_y, 
		...
		if ( csc.dbg_flag & DBG_MT_STEP1 )
			dbg_p_d_data_f_mn ( "motion 1",( float *)d_currp,
				( rec_size + NUM_OF_HVT_INDEX ) * hvt_size * nblk_in_x * nblk_in_y, 
				rec_size + NUM_OF_HVT_INDEX, hvt_size, rec_size + NUM_OF_HVT_INDEX ) ;
				// rec_size + NUM_OF_HVT_INDEX, hvt_size, 15, 100 ) ;

		// step 2 -- do the sum

		omp_timer_on ( CS_TIMER_ANALYSIS_MD2 ) ;

		k = h_do_l1_norm_step2_v2<float>( ( float *)d_currp,
			rec_size * hvt_size * nblk_in_x * nblk_in_y, 
			rec_size, wcube_info, d_cubep, d_host_io ) ;
		...
		if ( csc.dbg_flag & DBG_MT_STEP2 )
			dbg_p_d_data_f_mn ( "motion 2", ( float *)d_currp,
				( rec_size + NUM_OF_HVT_INDEX ) * hvt_size * nblk_in_x * nblk_in_y, 
				rec_size + NUM_OF_HVT_INDEX, hvt_size, rec_size + NUM_OF_HVT_INDEX ) ;
				// rec_size + NUM_OF_HVT_INDEX, hvt_size, 15, 100) ;
		...
		k = h_do_l1_norm_step3_v2<float>((float *)d_currp, rec_size * hvt_size * nblk_in_x * nblk_in_y,
			rec_size, orig, hvt_size ) ;
		...
		if ( csc.dbg_flag & DBG_MT_STEP3 )
			dbg_p_d_data_f_mn ( "motion 3", ( float *)d_currp,
				( rec_size + NUM_OF_HVT_INDEX ) * hvt_size * nblk_in_x * nblk_in_y, 
		...
		k = h_do_l1_norm_step4_v2<float>( ( float *)d_currp,
			rec_size * hvt_size * nblk_in_x * nblk_in_y, 
			rec_size, orig, hvt_size, outbufp, ( csc.md_y * 2 + 1 + 1 ) * csc.md_x ) ;
				// the no motion idx ... seems not right, need to revisit ...
		...
		if ( csc.dbg_flag & DBG_MT_STEP4 )
			dbg_p_d_data_f_mn ( "motion 4", ( float *)d_currp,
				( rec_size + NUM_OF_HVT_INDEX ) * hvt_size * nblk_in_x * nblk_in_y, 
...
#ifdef CUDA_OBS 
		dbg_p_data_md_f_mn ( "return values", ( int *)outbufp,
			( 1 + NUM_OF_HVT_INDEX ) * nblk_in_x * nblk_in_y * 2, 
			( 1 + NUM_OF_HVT_INDEX ) * 2, nblk_in_x * nblk_in_y, ( 1 + NUM_OF_HVT_INDEX ) * 2) ;
#endif 
