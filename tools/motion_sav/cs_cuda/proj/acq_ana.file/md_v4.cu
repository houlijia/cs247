...
static struct cube *d_cubep = NULL ;	// cube in device 	// cube in device 
static int *d_host_io = NULL ;

// md v4
static int to_blk_entries_size = 0, to_blk_size = 0 ;
static int max_record_length = 0, max_loopcnt = 0 ; // include the tvh
...
static struct per_blk_data __pbd, *pbd_p = &__pbd ;
...
allocate_d_mem()
{
	...
	if ( csc.do_cube )
	{
		...
		for ( i = 0 ; i <  CUBE_INFO_CNT  ; i++ )
		{
			...
			cube_info[i].interval = ( float )csc.interval_factor[i] *
				( float )sqrt(( double )cube_info[i].size / ( double )12 ) ;

			printf("%s: i %d x/y/z %d %d %d interval %f \n", __func__, i, x, y, z,
				 cube_info[i].interval );
			...
			cube_info[i].md_v4_loopcnt = ( csc.md_x * 2 + 1 ) * ( csc.md_y * 2 + 1 ) *
				cube_info[i].md_v3_cnt + 1; 

			cube_info[i].md_v4_record_length = x * y * cube_info[i].md_v3_hv_cnt +
				NUM_OF_HVT_INDEX ;

			if ( max_record_length < cube_info[i].md_v4_record_length )
				max_record_length = cube_info[i].md_v4_record_length ;

			if ( max_loopcnt < cube_info[i].md_v4_loopcnt )
				max_loopcnt = cube_info[i].md_v4_loopcnt ;

			k = ( cube_info[i].md_v4_record_length - NUM_OF_HVT_INDEX ) * cube_info[i].md_v4_loopcnt ;
			if ( k > to_blk_entries_size )
				to_blk_entries_size = k ;

			k = cube_info[i].md_v4_record_length * cube_info[i].md_v4_loopcnt ;

				// last + 1 is for orig

			printf("%s: x %d y %d k %d ana_size %d v4_loop %d\n", __func__, x, y, k, ana_size,
				 cube_info[i].md_v4_loopcnt ) ;
			...
			if ( k <= 0 )
			{	
				printf("allocate_d_mem: allocate_d_mem failed \n") ;
				return ( 0 ) ;
			}
...
#ifdef CUDA_DBG 
		printf("max_loopcnt %d max_record_length %d \n", max_loopcnt, max_record_length ) ;
#endif 

		to_blk_size = ana_size ;
	...
	if ( csc.do_reconstruction )
	{
		if ( !h_do_dct_init())
		{
			printf("allocate_d_mem: h_do_dct_init failed \n") ;
			return ( 0 ) ;
		}
	}
	...
	if ( !csc.do_analysis )
	{
		j = sizeof ( float ) * nblk_in_x * nblk_in_y * NUM_F_IN_PBD + sizeof ( int )
			* nblk_in_x * nblk_in_y * NUM_I_IN_PBD ; 
		if (( i = cudaMalloc( &pbd_p->d_meanp, j )) != cudaSuccess )
		{
			printf("%s: d_meanp failed %d \n", __func__, i ) ;
			return ( 0 ) ;
		}
		pbd_p->d_sdp = pbd_p->d_meanp + nblk_in_x * nblk_in_y ;
		pbd_p->d_dcp = pbd_p->d_sdp + nblk_in_x * nblk_in_y ;
		pbd_p->d_interval = pbd_p->d_dcp + nblk_in_x * nblk_in_y ;
		pbd_p->d_ampl = pbd_p->d_interval + nblk_in_x * nblk_in_y ;
		pbd_p->d_offset = pbd_p->d_ampl + nblk_in_x * nblk_in_y ;

		pbd_p->d_max_bin = ( int * )(pbd_p->d_offset + nblk_in_x * nblk_in_y) ;
		pbd_p->d_num_bin = pbd_p->d_max_bin + nblk_in_x * nblk_in_y ;

		total_d_mem += j ;

		printf("%s ::: d_meanp %p d_sdp %p d_dcp %p interval %p\n", __func__,
			pbd_p->d_meanp,
			pbd_p->d_sdp,
			pbd_p->d_dcp,
		   	pbd_p->d_interval ) ;

		printf("	d_ampl %p d_offset %p d_num_bin %p max %p\n",
			pbd_p->d_ampl,
		   	pbd_p->d_offset,
	...
	int need_interpolate, struct frame_list *fp )
{
	...
	{
		// save the dc ...
		if ( !( k = h_do_copy_vec<float> ( ( float *)d_currp, ( float *)pbd_p->d_dcp,
			nblk_in_x * nblk_in_y, 
			perm_size_e,
			1 )))
		{
			printf("%s: save dc failed %d\n", __func__, k ) ;
			return ( 0 ) ;
		} 
#ifdef CUDA_OBS 
		dbg_p_d_data_f ( "from 0 dc ... ", ( float * )d_currp, 1 ) ;
		dbg_p_d_data_f ( "from 1 dc ... ", ( float * )d_currp + perm_size_e, 1 ) ;
		dbg_p_d_data_f ( "from last dc ... ", ( float * )d_currp +
			( nblk_in_x * nblk_in_y - 1 ) * perm_size_e, 1 ) ;
		dbg_p_d_data_f ( "dc ... ", ( float *)pbd_p->d_dcp, nblk_in_x * nblk_in_y ) ;
#endif 
	}
	...
	if ( csc.do_analysis )
	{
		...
		inner_cube_size = wcube_info[CUBE_INFO_INNER].size ;
		overall_size = inner_cube_size * nblk_in_x * nblk_in_y ;
		...
		k = h_do_motion_idx_v4 (( int *)d_nextp, blk_size_e,
			nblk_in_x, nblk_in_y, wcube_info,
			csc.md_x, csc.md_y, csc.md_z, d_cubep ) ;

#ifdef CUDA_DBG 
		printf("%s: nblk_in_x/y %d %d \n",
			__func__, nblk_in_x, nblk_in_y ) ;
#endif 
		...
		// the following data has a size issue ... it is too big ... for pets_test_enc case QQQ

		if ( csc.dbg_flag & DBG_MT_IDX )
			h_dbg_md_v4 ("after idx", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_TVH_IDX, 0, 0, 0 ) ;

		omp_timer_on ( CS_TIMER_ANALYSIS_MD0 ) ;

		// step 0 : copy data ...
		// currp has the valid edge data ... nextp has the valid (TVH) formated data
		k = h_do_motion_detection_step0_v4<float> (( float *) d_currp,( float *) d_nextp,
			wcube_info[CUBE_INFO_INNER].md_v4_loopcnt * 
				( wcube_info[CUBE_INFO_INNER].md_v4_record_length - CUBE_INFO_CNT ) *
				nblk_in_x * nblk_in_y,
			csc.md_x * 2, csc.md_y * 2, csc.md_z,
			d_cubep,
			wcube_info[ CUBE_INFO_INNER].size,	// from blk size
			to_blk_entries_size,
			to_blk_size,
			nblk_in_x, nblk_in_y ) ;
		...
		{
			h_dbg_md_v4 ("after step0", ( float *)d_nextp, wcube_info, to_blk_size,
				// nblk_in_x, nblk_in_y, P_TVH_IDX, 0, 0, 0 ) ;
				nblk_in_x, nblk_in_y, P_BLK, 287, 625, 630 ) ;

			h_dbg_md_v4 ("after step0", ( float *)d_nextp, wcube_info, to_blk_size,
				// nblk_in_x, nblk_in_y, P_TVH_IDX, 0, 0, 0 ) ;
				nblk_in_x, nblk_in_y, P_BLK, 288, 625, 630 ) ;
		}

		d_currp = d_nextp ;	// never needs the other buf ...
		...
		{
			h_dbg_md_v4 ("before step1", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 287, 625, 630 ) ;

			h_dbg_md_v4 ("before step1", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 287, 1250, 1250 ) ;
		}

		omp_timer_on ( CS_TIMER_ANALYSIS_MD1 ) ;

		k = wcube_info[CUBE_INFO_INNER].md_v4_loopcnt * 
				( wcube_info[CUBE_INFO_INNER].md_v4_record_length - CUBE_INFO_CNT ) ;

		h_do_l1_norm_step1_v4<float>((float *) d_currp, 
			k * nblk_in_x * nblk_in_y,
			k, 
			to_blk_size,
			d_cubep, nblk_in_x, nblk_in_y, 
			max_record_length ) ;
		...
		if ( csc.dbg_flag & ( DBG_MT_STEP1 ))
		{
			h_dbg_md_v4 ("after step1", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 287, 625, 630 ) ;
		...
		{
			h_dbg_md_v4 ("after step1", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 150, 625, 625 ) ;

			h_dbg_md_v4 ("after step1", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 150, 1250, 1250 ) ;
		}
...
#ifdef CUDA_OBS 
		k = h_do_l1_norm_step2_v4_block_v2<float>( ( float *)d_currp,
			wcube_info,
			d_cubep,
			nblk_in_x,
			nblk_in_y,
...
#ifdef CUDA_OBS 
		k = h_do_l1_norm_step2_v4_block_v1<float>( ( float *)d_currp,
			wcube_info,
			d_cubep,
			nblk_in_x,
			nblk_in_y,
...
#ifdef CUDA_OBS 
		k = h_do_l1_norm_step2_v4_block<float>( ( float *)d_currp,
			wcube_info,
			d_cubep,
			nblk_in_x,
			nblk_in_y,
		...
		k = h_do_l1_norm_step2_v4<float>( ( float *)d_currp,
			wcube_info,
			d_cubep,
			nblk_in_x,
			nblk_in_y,
		...
		if ( csc.dbg_flag & DBG_MT_STEP2 )
		{
			h_dbg_md_v4 ("after step2", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_BLK, 150, 625, 625 ) ;
...
#ifdef CUDA_OBS 
			h_dbg_md_v4 ("after step2", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 700, 702 ) ;

			h_dbg_md_v4 ("after step2", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 1250, 1250 ) ;
#endif 
			h_dbg_md_v4 ("after step2", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 0, 1250 ) ;
		}

		omp_timer_on ( CS_TIMER_ANALYSIS_MD3 ) ;

		k = h_do_l1_norm_step3_v4<float>((float *)d_currp,
			   ( max_loopcnt - 1 ) * nblk_in_x * nblk_in_y,
			   max_loopcnt - 1,
			   d_cubep,
			   to_blk_size,
			   nblk_in_x,
		...
		{
#ifdef CUDA_OBS 
			h_dbg_md_v4 ("after step3", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 700, 702 ) ;
		...
		{
#ifdef CUDA_OBS 
			// step 4.1 and 4.2 ... match below
			k = ( csc.md_y * 2 + 1 + 1 ) * csc.md_x ;

			h_dbg_md_v4 ("before step4", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, k-10, k+10 ) ;

			h_dbg_md_v4 ("before step4", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 1250, 1250 ) ;	// should be wcube.loopcnt
#endif 
#ifdef CUDA_OBS 
			h_dbg_md_v4 ("before step4", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 0, 1250 ) ;	// for step 4.3
#endif 
		}

		omp_timer_on ( CS_TIMER_ANALYSIS_MD4 ) ;

		k = h_do_l1_norm_step4_v4<float>( ( float *)d_currp, 1, // 1 place holder 
			outbufp, 
			( csc.md_y * 2 + 1 + 1 ) * csc.md_x,
			d_cubep,
			to_blk_size,
			nblk_in_x, nblk_in_y,
		    wcube_info ) ;
		...
		{
#ifdef CUDA_OBS 
			// step 4.1 4.2 ... match above
			k = ( csc.md_y * 2 + 1 + 1 ) * csc.md_x ;

			h_dbg_md_v4 ("after step4", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, k-10, k+10 ) ;
			...
			h_dbg_md_v4 ("after step4", ( float *)d_nextp, wcube_info, to_blk_size,
				nblk_in_x, nblk_in_y, P_ROW, 150, 0, 1250 ) ;	// for step 4.3
#endif 
			dbg_p_data_md_f_mn ( "after step4 return values", ( int *)outbufp,
...
#ifdef CUDA_OBS 
		dbg_p_d_data_f ( "value ... ",( float *)d_currp + ( inner_cube_size * 
			( nblk_in_x * 3 / 2 )), wcube_info[0].size ) ;
#endif 

		h_mean_sd (( float *) d_currp, ( float *)d_nextp, wcube_info, d_cubep,
			nblk_in_x, nblk_in_y, wcube_info[0].size, pbd_p->d_meanp, pbd_p->d_sdp ) ;

#ifdef CUDA_OBS 
		dbg_p_d_data_f ( "value ... ",( float *)d_currp + ( inner_cube_size * 
			( nblk_in_x * 3 / 2 )), wcube_info[0].size ) ;

		dbg_p_d_data_f ( "mean ... ", d_meanp, nblk_in_x * nblk_in_y ) ;
		dbg_p_d_data_f ( "sd ... ", d_sdp, nblk_in_x * nblk_in_y ) ;
#endif 

		// valid values are still in d_currp
		

		// QQQ

		h_do_quant(( float *)d_currp, ( int *)d_nextp, wcube_info[0].size, wcube_info, d_cubep,
			nblk_in_x, nblk_in_y,
			pbd_p->d_meanp, pbd_p->d_sdp, 
			pbd_p->d_max_bin, pbd_p->d_num_bin, pbd_p->d_ampl, pbd_p->d_offset,
			pbd_p->d_dcp ) ;
