function FindMultiBestfromDB(CM1,J1,FileNameO,FileNameAlert)
...
% BGRSize = 512*512*3; % Modify this!
% BGR = fread(fid,BGRSize,'uint8'); % Modify this!
% fclose(fid);
% 
% % I = BGR2I(BGR);
% I = reshape(BGR,[512 512 3]);
...
J = reshape(BGR,[512 512 3]);



fid = fopen(CM1,'r');
BGRSize = 128*128*6; % Modify this!
BGR = fread(fid,BGRSize,'int32'); % Modify this!
fclose(fid);

for i = 1 : 6
    for j = 1 : 128
        for k = 1 : 128
            CM{i}(j,k) = BGR(128^2*(i-1)+128*(j-1)+k);
        end
    end
end
