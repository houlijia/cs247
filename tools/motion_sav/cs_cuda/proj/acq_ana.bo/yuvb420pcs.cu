...
main( int ac, char *av[] )
{
	...
	if ( csc.video_source == VIDEO_SRC_FILE ) 
	{
		fin = open( csc.video_src, O_RDONLY ) ;
	...
	// STEP 5 ::: if config'ed, initialize the video camera

	if (( csc.video_source == VIDEO_SRC_IPCAM ) ||
		( csc.video_source == VIDEO_SRC_WEBCAM ))
	{
		if ( !cs_ipcam_init ( csc.z_block, csc.frame_x, csc.frame_y, csc.video_src,
