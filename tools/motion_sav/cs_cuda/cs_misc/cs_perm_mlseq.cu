	...
	int *d_rightp )
{
	...
	// *lp++ = 0 ; mlseq for the left is only 2^n -1 in size
	*rp++ = 0 ;
...
template<typename T>
__global__ void d_do_permutation_R ( T *input, T *output,
...
// Target(Perm(i)) = Orig(i)

template< typename T >
void
h_do_permutation_R ( T *d_input, T *d_output, int *d_perm_tbl,
	int n, int tbl_size )
{
	...
	d_do_permutation_R<T> <<< nBlocks, nThreadsPerBlock >>> ( d_input,
...
template void
h_do_permutation_R<int> ( int *d_input, int *d_output, int *d_perm_tbl,
	int n, int tbl_size ) ;

template void
h_do_permutation_R<float> ( float *d_input, float *d_output, int *d_perm_tbl,
	int n, int tbl_size ) ;
 
template<typename T>
__global__ void d_do_permutation_Lv2 ( T *input, T *output,
...
/* 
   d_perm_tbl_i/s/c : the perm table on device for inner/side/corner blocks
   tbl_size is the size for each block ( mod 2 ), n is the overall size,
   i.e. n * nblk_in_x * nblk_in_y
*/

template<typename T>
void
h_do_permutation_Lv2 ( T *d_input, T *d_output,
	...
	int nblk_in_x, int nblk_in_y )
{
	...
	d_do_permutation_Lv2<T> <<< nBlocks, nThreadsPerBlock >>> ( d_input,
...
template void h_do_permutation_Lv2<int> ( int *d_input, int *d_output,
	int *d_perm_tbl_i, int *d_perm_tbl_s, int *d_perm_tbl_c, int n, int tbl_size,
	int nblk_in_x, int nblk_in_y ) ;

template void h_do_permutation_Lv2<float> ( float *d_input, float *d_output,
	int *d_perm_tbl_i, int *d_perm_tbl_s, int *d_perm_tbl_c, int n, int tbl_size,
	int nblk_in_x, int nblk_in_y ) ;
	...
	int *d_rightp )
{
	...
	if ( fd < 0 )
	{
	  printf("%s:open L %s fail: %s\n", __func__, fipL,strerror(errno)) ;
		goto fail_out ;
	}
	...
	if ( fd < 0 )
	{
	  printf("%s:open R %s fail: %s\n", __func__, fipR, strerror(errno)  ) ;
		goto fail_out ;
	}
