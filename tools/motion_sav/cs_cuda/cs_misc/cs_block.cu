...
template<typename T>
__global__ void d_make_block ( T *input, T *output,
		...
		if ( frame_num_in_blk < zbdim )
		{
			...
			if ( pix_row_in_blk >= app_x )
			{
				...
				if ( pix_column_in_blk < xbdim )
				{
					...
					output[ t_idx ] = (( input[ i ] * (( T ) weight )) / jj ) ;
				}
			}
		}
...
h_make_block does 3 things ... ( good idea? )
	1. append 0 on x/y direction ...
	2. weighing, if so configured
	3. cut into blocks ... 
...
blk_dst_size : block size, in elements, on device, including padding
	max_log2(( xbdim + app_x ) * ( ybdim + app_y ) * zbdim )  
...
template< typename T >
void
h_make_block ( T *d_input, T *d_output,
	int xdim, int ydim, int frame_size,
	...
	)
{
	...
	d_make_block<T> <<< nBlocks, nThreadsPerBlock >>> ( d_input, d_output,
...
template void 
h_make_block<int> ( int *d_input, int *d_output,
	int xdim, int ydim, int frame_size,
	int xbdim, int ybdim, int zbdim, int blk_dst_size,
	int do_perm,
	int x_overlap, int y_overlap,
...
template void 
h_make_block<float> ( float *d_input, float *d_output,
	int xdim, int ydim, int frame_size,
	int xbdim, int ybdim, int zbdim, int blk_dst_size,
	int do_perm,
	int x_overlap, int y_overlap,
