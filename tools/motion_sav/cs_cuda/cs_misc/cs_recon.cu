...
{
	int i, row, col ;
	float *rp=NULL, *gp=NULL, *bp=NULL, *fp ;
	...
	for ( i = 0 ; i < RGB_CNT ; i++ )
	{
		...
		case 2 :
#ifdef CUDA_DBG 
			dbg_p_d_data_f("BBBBB", fp, 8 * 8 ) ;
#endif 
			bp = fp ;
			break ;
		}

		p_buffer_dbg("after TV_GAP swap buf") ;
	...
	printf("NEED MORE WORK ... the following should be in GPU ... \n") ;

	// find the max of r.g.b. ... and squeeze ...

	fp = h_cropping ( rp, pp->wht_size, pp->c, pp->r, pp->r_start, pp->c_start ) ;

	if ( fp == NULL )
	{ 
		printf("%s : rp cropping failed \n", __func__ ) ;
		cs_put_free_list(( char *) rp, 0 ) ;
		return ( NULL ) ;
	}
	rp = fp ;

	fp = h_cropping ( gp, pp->wht_size, pp->c, pp->r, pp->r_start, pp->c_start ) ;

	if ( fp == NULL )
	{ 
		printf("%s : gp cropping failed \n", __func__ ) ;
		cs_put_free_list(( char *) gp, 0 ) ;
		return ( NULL ) ;
	}
	gp = fp ;

	fp = h_cropping ( bp, pp->wht_size, pp->c, pp->r, pp->r_start, pp->c_start ) ;

	if ( fp == NULL )
	{ 
		printf("%s : bp cropping failed \n", __func__ ) ;
		cs_put_free_list(( char *) bp, 0 ) ;
		return ( NULL ) ;
	}
	bp = fp ;
	...
	if (!dbg_copy_d_data(( char *)fp, ( char *)gp, sizeof ( float ) * i ))
	{
		printf("%s : bp cropping failed \n", __func__ ) ;
		cs_put_free_list(( char *) fp, 0 ) ;
		return ( NULL ) ;
	...
	max_g = h_do_max_destroy ( fp, i ) ;

	if (!dbg_copy_d_data( ( char *)fp, ( char *)bp, sizeof ( float ) * i ))
	{
		printf("%s : bp cropping failed \n", __func__ ) ;
		cs_put_free_list(( char *) fp, 0 ) ;
		return ( NULL ) ;
...
#ifdef CUDA_OBS 
	dbg_p_d_data_f ("RRR--", rp, pp->c * pp->r );
	dbg_p_d_data_f ("GGG--", gp, pp->c * pp->r );
	dbg_p_d_data_f ("BBB--", bp, pp->c * pp->r );
#endif 
#endif 
	...
	h_a2d ( bp, ( unsigned char *)fp, 0, i, max_b ) ;
	h_a2d ( gp, ( unsigned char *)fp, 1, i, max_g ) ;
	h_a2d ( rp, ( unsigned char *)fp, 2, i, max_r ) ;

	*outsize = col * row * RGB_CNT ;

	printf("row %d col %d size %d \n", row, col, *outsize ) ;

	dbg_get_d_data(( char *) fp, ( char *)rgbp, col * row * RGB_CNT ) ;
