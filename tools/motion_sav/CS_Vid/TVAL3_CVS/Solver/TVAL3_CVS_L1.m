function [U, out] = TVAL3_CVS_L1(A,b,p,q,r,opts)
%
% L1 minimization algorithm for 3D data modified from TVAL3.
%
% Goal: solve   min sum ||u||_1    or    min sum ||DCT(u)||_1
%                  s.t. Au = b              s.t. Au = b
...
if opts.aqst
    D = @(U) Id(mirt_dctn(U));        % using 3D DCT as sparsity basis
    Dt = @(X,Y) mirt_idctn(Idt(X,Y));
else
    D = @(U) Id(U);
    Dt = @(X,Y) Idt(X,Y);
end
