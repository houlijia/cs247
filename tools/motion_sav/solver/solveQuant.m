...
%   xerr.wdthAerr = 0;
  ...
  [xerr.D, xerr.J] = optimize_solver_w(xvec, sparser, ...
    beta.D, lambda.D, wvec_ref);
  [grad_x, Agrad_x, pAgrad_x, Dgrad_x, cjerrD, gLg] = ...
  ...
  for ext_iter = 1:opts.max_out_iters
    ...
    for int_iter = 1:min(opts.max_int_iters, opts.max_iters-iter.C)
      ...
      if ~init_cg
        gLd = beta.D * dot(Dgrad_x, Ddrct) + ...
          beta.scldA*dot(pAgrad_x,Adrct);
        past_wgt = gLd/dLd;
      ...
      if iter.I > 1 && abs(step_size) <= opts.step_ratio_thrsh * prev_step_size
        ...
        if do_disp
          ...
          fprintf(['%s~~ dLd=%10.3g '...
            'max_err=(%10.3g,%10.3g) DONE \n'], prfx(),...
            dLd, norm(xerr.A,inf), norm(xerr.D,inf));
        end
      ...
      [lgrngn1, xvec1, xerr1] =...
        move_step(xvec, xerr, drct, Adrct, step_size,...
      ...
      if do_disp
        fprintf(['%s~ J_opt=%12.05g step_size=%6.4f '...
          ' max_err=(%10.3g %10.3g) sqr_err=(%10.3g %10.3g)\n'], prfx(),...
          xerr.J, step_size, ...
    ...
    if disp_psnr || do_disp
      ...
      if ~isempty(psnr)
        ok_str = sprintf('PSNR=%4.1f, %s', cmp_blk_psnr(xvec), ok_str);
      end
...
function [grad_x, Agrad_x, pAgrad_x, Dgrad_x, gLg] = updateGrad_x(...
    ...
    gLg = beta.D * dot(Dgrad_x, Dgrad_x) + beta.scldA*dot(pAgrad_x,pAgrad_x);
end
