        ...
        
{
    ...
    p = ceil(log(m)/log(2));
    N = 1 << p;
#else /* Razi Haimi-Cohen: Unfortunately, microsoft eliminated the log2() function. I guess that this is
         more efficient than the previous implementation with log(m)/log(2) */
    for(N=1; N<m; N=N<<1, p+=1);
#endif
