...
// the blk here refers to the L-selection block ...
...
__global__ void d_do_edge_detection_v2 ( int *fdp, int *tdp, int tbl_size, 
	struct cs_xyz *xyzp, int ex, int ey, int bxyz_size, int exy_size,
	int blk_in_x, int blk_in_y )
{
	int t_idx = blockIdx.x * blockDim.x + threadIdx.x;
	int *ofdp, cx, cy, cz, mea, xy_size, xyz_idx, *fp, x, y, frame, block, sum, i, j ;
	double d ;
...
// tbl size is cube_x * cube_y * cube_z * nblok_in_x * nblock_in_y

int
h_do_edge_detection_v2 ( int *fromp, int *top, int tbl_size,
	...
	int blk_in_y, struct cube *cubep )
{
	...
	d_do_edge_detection_v2 <<< nBlocks, nThreadsPerBlock >>> (
