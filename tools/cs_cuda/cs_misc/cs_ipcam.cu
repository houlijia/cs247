...
int sem_lock_init() ;
void setup_window() ;
void frame_plist( const char *s ) ;
int frame_list_init ( int cnt, int gsize, int vsize, int bsize ) ;
void *frame_grabber( void * ) ;
void * frame_displayer( void *p) ;
void sem_info( const char *s ) ;

static int sizeVideo, sizeGray ;
static int frame_per_io = 0 ;
static CvCapture *videoIn ;
static int htVideo, widVideo, fps, ms_wait ;
...
cs_ipcam_init( int frame_in_block, int width, int height, char *axis, int blkx, int blky,
	int md_xx, int md_yy, int display_threshold_x, int display_threshold_y )
{
	int fps ;
...
struct frame_list_head free_frame_head ;
struct frame_list_head filled_frame_head ;
struct frame_list_head display_frame_head ;
...
frame_displayer( void *p)
{
	...
	int *dp, i, j, k, h, v, t, va, oh, ov, ot, ova ;
