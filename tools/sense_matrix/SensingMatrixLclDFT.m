classdef SensingMatrixLclDFT < SensingMatrixSqrLclRnd & SensingMatrixDFT
  %SensingMatrixLclDFT Same as SensingMatrixDFT, except that the
  ...
  methods
    ...
    %     inp_list: A list of input measurement numbers to use. 'all'
    %               indicates all measurements
    %     params: A struct with the possible fields:
    %               nrm_exp -
...
%       vmsrs = vmsrs .* scaler;
  ...
  methods(Access=protected)
    function PL = makeRowSlctPermutation(obj, order, opts)
      PL = obj.makeRowSlctPermutation@SensingMatrixDFT(order, opts);
    end
