...
% For each image, the running time is about 10 minutes
...
[stats, enc_opts] =  run_quant_test_cscd_test_2D_DCTWHT_GAPTV('../test_data/Image', '../test_data/Image',img_files);
...
legend('JPEG', 'JPEG2000','CS (GAP-TV), 2D-WHT','CS (GAP-TV), 2D-DCT','Location','southeast');
set(gca,'FontSize',12)
xlabel('Compressed file size (Bytes)','FontSize',12)
ylabel('Structural Similarity (SSIM)','FontSize',12)
 title(filename)
saveas(h, [filename '_JPEG_CS_GAPTV_2D_WHTDCT.fig']);
saveas(h, [filename '_JPEG_CS_GAPTV_2D_WHTDCT.png']);
...
pause(1);
save('8image_gap_tv_result.mat','-v7.3','stats_save','sav_bytes_save_jpeg','save_ssim_save_jpeg',...
