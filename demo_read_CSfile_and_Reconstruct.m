% Please run "demo_GAPTV' before running this code as this code will read
% the CS file generated by "demo_GAPTV"
%=============================
...
% After we run "demo_GAPTV.m", we have the CS file stored at 
...
% Please run "demo_GAPTV' before running this code as this code will read
% the CS file generated by "demo_GAPTV"
% This file will reproduce figure 4 in the main paper is nothing is changed
% and run all the images
...
for nim = 1:8
    ...
    ylabel('Structural Similarity (SSIM)','FontSize',12)
     title(filename);
     legend('JPEG', 'JPEG 2000', 'CS (GAP-TV), 2D-WHT','CS (D-AMP), 2D-WHT','CS (NLR-CS), 2D-WHT',...
...
figure(h8);
legend('JPEG', 'JPEG 2000', 'CS (GAP-TV), 2D-WHT','CS (D-AMP), 2D-WHT','CS (NLR-CS), 2D-WHT',...
    'CS (GAP-TV), 2D-DCT','CS (D-AMP), 2D-DCT','CS (NLR-CS), 2D-DCT');

save(['.\Temp_results\' filename '_all3_2D_DCTWHTsensing_readfile_rec.mat'], '-v7.3','im_rec','PSNR_all','SSIM_all','filesize');
